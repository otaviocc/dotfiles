...
| holodeck | built-in — `"theme": "catppuccin-mocha"` in `config.json` |
...
| Claude Code | vendored as `claude/.claude/themes/catppuccin-mocha.json` (`base: dark-ansi` + Mocha accent overrides) |
...
- **The `claude` package tracks `themes/` only, never `settings.json`.** That
  file is what *selects* the theme, but it also holds API tokens, so it must
  stay out of the repo. After a fresh stow the theme file is in place but
  unselected — pick it with `/theme`, or set
  `"theme": "custom:catppuccin-mocha"` in `~/.claude/settings.json` by hand.
  `~/.claude/themes/default-plus.json` is untracked too.
